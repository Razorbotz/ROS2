...
class Teleop(Node):
    ...
    def turn_90_degrees(self):
        ...
        while rclpy.ok():
            ...
            # Print error to debug (optional, but helpful)
            ...
            # 1. Increased KP (Was 0.5)
            # This makes the robot react stronger to error
            speed = error * 3.0 
            
            # 2. Increased Max Turn Speed (Was 5.0)
            MAX_VAL = 10.0
            speed = max(min(speed, MAX_VAL), -MAX_VAL)
            
            # 3. CRITICAL: Increased Minimum "Break-Away" Speed
            # 1.0 is too weak to skid the tires. 
            # We raise this to 4.0 to force the tires to slide.
            MIN_VAL = 4.0
