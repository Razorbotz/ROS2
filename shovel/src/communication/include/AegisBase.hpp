...
public:
    ...
    void sendAuthResponse(bool granted);
    ...
    // ---- Motor Authorization (Status Monitor + Node Liveness) ----
     ...
     *
     * @param can0_states Array indicating CAN0 detection per motor index (0-7).
     * @param can1_states Array indicating CAN1 detection per motor index (0-7).
     */
    void processStatusMonitorCANReport(const uint8_t can0_states[MAX_MOTORS],
                                       const uint8_t can1_states[MAX_MOTORS]);

    /**
     * @brief Called when any motor node message is received.
    ...
    /**
     * @brief Returns whether a given motor's node is currently alive
     * based on the per-motor node active flags.
     *
     * @param motor_index The 0-based motor index (0-7).
     * @return true if the motor node is considered alive.
     */
    bool isMotorNodeAlive(uint8_t motor_index) const;
